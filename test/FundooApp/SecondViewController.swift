...
import CoreData
import Alamofire
struct ReminderNotess:Encodable{
    
    let reminder:String?
    let noteIdList:[String]?
}
struct CreateNotee:Encodable{
    let title:String?
...
//    @IBOutlet weak var trailing: NSLayoutConstraint!
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        //deletecoredata()
        //UserDefaults.standard.removeObject(forKey: "checknet")
...
//        }
        ...
        {
            var myarray1 = UserDefaults.standard.stringArray(forKey: "date")
            var myarray2 = UserDefaults.standard.stringArray(forKey: "NoteeId")
            
            for i in 0..<myarray1!.count
            {
                print("138...............\(myarray1?.count)")
                var id = UserDefaults.standard.string(forKey: "idforlogin")
                var remindernote = ReminderNotes(reminder: myarray1![i], noteIdList: [myarray2![i]])
        ...
        if UserDefaults.standard.bool(forKey: "checknotee")
        {
            var myarray1 = UserDefaults.standard.stringArray(forKey: "titlee")
            var myarray2 = UserDefaults.standard.stringArray(forKey: "descriptioon")
            var myarray3 = UserDefaults.standard.stringArray(forKey: "colorr")
            for i in 0..<myarray1!.count
            {
                print("161..............\(myarray1?.count)")
                var id = UserDefaults.standard.string(forKey: "idforlogin")
                //var remindernote = ReminderNotes(reminder: myarray1![i], noteIdList: [myarray2![i]])
                var createnote = CreateNotee(title: myarray1![i], description: myarray2![i], color: myarray3![i])
                AF.request("http://fundoonotes.incubation.bridgelabz.com/api/notes/addNotes?access_token="+id!,
                           method: .post,
                           parameters: createnote,
                           encoder: JSONParameterEncoder.default).response { response in
                            debugPrint(response)
                }
            }
        }
        if UserDefaults.standard.bool(forKey: "deletecheck")
        {
            var myarray1 = UserDefaults.standard.stringArray(forKey: "noteeide")
            for i in 0..<myarray1!.count
            {
                 var id = UserDefaults.standard.string(forKey: "idforlogin")
                var NoteID = ["noteIdList" : [myarray1![i]]]
            ...
            do {
                ...
                for i in 0 ..< result.count{
                    ...
                    if n != nil {
                        ...
                        //print("b is \(b)")
                        var text3 = Text(texttitle: n!, textnote: m!, index: i, important: b!, archive: arch!, id: "abc", color: color)
                        //tempArray.append(text3)
